...
# biến lưu trữ giá trị được chọn và lưu ở giá trị kiểu số nguyên (biến lưu trữ Radio Button)
v = tk.IntVar()
...
def click():
    print(f"Bạn đang chọn ngôn ngữ có ID: {v.get()}")  # Lấy giá trị hiện tại của Radiobutton đc chọn

# tạo nhãn tiêu đề
tk.Label(root, text= " hãy chọn phần mềm bạn cần", justify=tk.LEFT, # justify:dùng để căn chỉnh nội dung nhiều dòng
         ).pack()
...
for name, val in danh_sach:
    tk.Radiobutton(root, 
                   ...
                   padx=20, 
                   variable=v,      # tất cả các liên kết với cùng 1 biến v, nên chỉ chọn đc 1 cái
                   command=click,
